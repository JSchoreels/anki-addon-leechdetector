...
class LapseInfos:
    ...
    def __init__(self, card_id : CardId, past_max_intervals : list[int], current_lapse_max_performance : int = 0, date_first_review : int = 0, review_count : int = 0, improvement_factor : float = 1):
        self.card_id = card_id
        self.past_max_intervals = past_max_intervals
        self.date_first_review = date_first_review
        self.review_count = review_count
        self.current_lapse_max_performance = current_lapse_max_performance
    ...
    def is_recovering_leech(self):
        return self.is_leech() and self.current_lapse_max_performance > max(self.past_max_intervals) and not self.is_recovered_leech()

    def is_recovered_leech(self):
        return self.is_leech() and self.current_lapse_max_performance > max(self.past_max_intervals) * 2

    def days_above_past_max_intervals(self):
        return self.current_lapse_max_performance - max(self.past_max_intervals)
    ...
    def __repr__(self):
        if self.lapses_count > 0:
            return f'Card : {self.card_id} Days:{self.date_first_review:3d} Reviews:{self.review_count}({self.date_first_review/self.review_count:3.2f}d/r Lapses:{self.lapses_count}) PastLapses:{self.past_max_intervals} (now:{self.current_lapse_max_performance}({self.days_above_past_max_intervals():+d}))  Drops:{self.performance_drop_count()} FailedImprovement:{self.failed_improvement_count()} FailedImprovementRatio:{self.failed_improvement_ratio() * 100:.2f}'
        else:
            return f'Card : {self.card_id} Days:{self.date_first_review:3d} Reviews:{self.review_count}({self.date_first_review/self.review_count:3.2f}d/r) No Lapse, Current Max Perf. : {self.current_lapse_max_performance}'
    ...
    def to_dict(self):
        return {
            ...
            "current_lapse_max_performance": self.current_lapse_max_performance,
        }
